...
func setupTestDB(t *testing.T) *sql.DB {
	db, err := sql.Open("pgx", "host=localhost user=postgres password=saleh dbname=postgres port=5433 sslmode=disable")
	if err != nil {
...
func TestCreatworkout(t *testing.T) {
	db := setupTestDB(t)
	defer db.Close()
	storee := NewPostgresWorkoutStore(db)
	tests := []struct {
		name    string
		workout *Workout
		wantErr bool
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			createdWorkout, err := storee.CreateWorkout(tt.workout)
			if tt.wantErr {
				assert.Error(t, err)
				return
			}
			...
			retrieved, err := storee.GetWorkoutByID(int64(createdWorkout.ID))
			require.NoError(t, err)
