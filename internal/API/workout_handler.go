...
type WorkoutHandler struct {
	workoutStore storee.WorkoutStore
	logger       *log.Logger
}

func NewWorkoutHandler(workoutStore storee.WorkoutStore, logger *log.Logger) *WorkoutHandler {
...
func (wh *WorkoutHandler) HandleGetWorkoutByID(w http.ResponseWriter, r *http.Request) {
	workoutID, err := utils.RedIDParam(r)
	if err != nil {
	...
	// fmt.Fprintf(w, "this is the workout id %v\n", workoutID)
...
// 	var workout storee.Workout
// 	err := json.NewDecoder(r.Body).Decode(&workout)
// 	if err != nil {
// 		wh.logger.Printf("ERROR: decodingCreatWorkout: %v", err)
// 		utils.WriteJSON(w, http.StatusBadRequest, utils.Envelope{"error": "invalid request sent"})
// 		return
// 	}

// 	currentUser := middleware.GetUser(r)
// 	if currentUser == nil || currentUser == storee.AnonymounsUser {
...
// 		wh.logger.Printf("ERROR: creatWorkout: %v", err)
// 		utils.WriteJSON(w, http.StatusInternalServerError, utils.Envelope{"error": "failed to creat workout"})
// 		return
// 	}
...
func (wh *WorkoutHandler) HandleCreateWorkout(w http.ResponseWriter, r *http.Request) {
	...
	currentUser := middleware.GetUser(r)
	if currentUser == nil || currentUser == storee.AnonymousUser {
		utils.WriteJSON(w, http.StatusBadRequest, utils.Envelope{"error": "you must be logged in"})
		return
	}
...
func (wh *WorkoutHandler) HandleUpdateWorkoutByID(w http.ResponseWriter, r *http.Request) {
	...
	// at this point we can assume we are abele to find and existion workout
	var updateWorkoutRequest struct {
		Title           *string               `json:"title"`
		Description     *string               `json:"description"`
		DurationMinutes *int                  `json:"duration_minutes"`
		CaloriesBurned  *int                  `json:"calories_burned"`
		Entries         []storee.WorkoutEntry `json:"entries"`
	}
